...
class GBV():
    def __init__(self):
        ...
        self.puzzles  =  [c for c in  re.split('[, \t]', open('puzzle', encoding='utf-8').read()) if len(c)]
        self.pre_ans  =  'wtf'
        self.answer   =  'wtf'
        self.winner   =  0
        self.last_painter = 0
    ...
    def guess(self, uid, tar):
        print('{} guess {}'.format(self.players[uid].name, tar))
        if tar != self.answer: 
            self.players[uid].coin -= 1
            return {'res' : 1}
        self.winner = uid
        self.players[uid].coin += 1
        self.players[self.painter].coin += 1
        self.stop_game()
        return { 'res' : 0 }
...
class S(Base_handler):
    def run(self, data):
        ...
        if data['cmd'] == 'reg':
            ...
            return G.guess(data['uid'], data['tar'])
